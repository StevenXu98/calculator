...
void getexpression()
{
	 ...
	 while((*p = getchar()) != '\n'){
	 	if(*p != ' '){
	 		if(isupper(*p)) *p = tolower(*p);//将大写转化为小写
	 	p++;
	 	limit++;
	 	}
	 }
...
char Prioritize(ElemenType *operator1, ElemenType *operator2)
{
	...
	if(!strcmp(oper1,"ln")||!strcmp(oper1,"lg")||
		...
		switch(oper1[0]){
			...
			case '*': case '/': return (!strcmp(oper2,"+")||!strcmp(oper2,"-")||!strcmp(oper2,"*")||
										!strcmp(oper2,"/")||!strcmp(oper2,")")||!strcmp(oper2,"=")) ? '>' : '<';
...
ElemenType Calculate(ElemenType num1, ElemenType oper, ElemenType num2)
{
	ElemenType result;
	if(oper.type == 1){
		if(!strcmp(oper.operator,"!")) result.number = factorial(num2.number);
		else if(!strcmp(oper.operator,"lg")) result.number = log10(num2.number);
		else if(!strcmp(oper.operator,"ln")) result.number = log(num2.number);
		else if(!strcmp(oper.operator,"sin")) result.number = sin(num2.number);
		else if(!strcmp(oper.operator,"cos")) result.number = cos(num2.number);
		else if(!strcmp(oper.operator,"tan")) result.number = tan(num2.number);
	}else{
...
int main()
{
	...
	scanf("%d",&mode);
	fflush(stdin);
	if(mode == 2) freopen("input","r",stdin);//重新定义输入流，实现文件操作
		printf("\n输入表达式，回车求值 \n");
		printf("输入quit结束程序\n\n\n");
